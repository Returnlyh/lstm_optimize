...
def read_tflite(tflite_name, log_func=print):
    ...
    for idx in range(len(subg['operators'])):
        ...
        if layer_name == "CONV_2D" or layer_name == "DEPTHWISE_CONV_2D":
            ...
            print("[INFO]>>> FULLY_CONNECTED ok!!!!")

        elif layer_name == "UNIDIRECTIONAL_SEQUENCE_LSTM":
            w_ix_idx = input_tensor_idx[1]
            w_fx_idx = input_tensor_idx[2]
            w_cx_idx = input_tensor_idx[3]
            w_ox_idx = input_tensor_idx[4]

            w_ih_idx = input_tensor_idx[5]
            w_fh_idx = input_tensor_idx[6]
            w_ch_idx = input_tensor_idx[7]
            w_oh_idx = input_tensor_idx[8]

            b_i_idx = input_tensor_idx[12]
            b_f_idx = input_tensor_idx[13]
            b_c_idx = input_tensor_idx[14]
            b_o_idx = input_tensor_idx[15]

            w_ix = interpreter.get_tensor(w_ix_idx)
            w_fx = interpreter.get_tensor(w_fx_idx)
            w_cx = interpreter.get_tensor(w_cx_idx)
            w_ox = interpreter.get_tensor(w_ox_idx)

            w_ih = interpreter.get_tensor(w_ih_idx)
            w_fh = interpreter.get_tensor(w_fh_idx)
            w_ch = interpreter.get_tensor(w_ch_idx)
            w_oh = interpreter.get_tensor(w_oh_idx)

            b_i = interpreter.get_tensor(b_i_idx)
            b_f = interpreter.get_tensor(b_f_idx)
            b_c = interpreter.get_tensor(b_c_idx)
            b_o = interpreter.get_tensor(b_o_idx)

            w_x = np.concatenate((w_ix, w_fx, w_cx, w_ox), axis=0)
            w_h = np.concatenate((w_ih, w_fh, w_ch, w_oh), axis=0)
            bias = np.concatenate((b_i, b_f, b_c, b_o), axis=0)

            l.update(
                {
                    # "w_ix":w_ix, "w_fx":w_fx, "w_cx":w_cx, "w_ox":w_ox,
                    # "w_ih":w_ih, "w_fh":w_fh, "w_ch":w_ch, "w_oh":w_oh,
                    # "b_i":b_i, "b_f":b_f, "b_c":b_c, "b_o":b_o
                    "w_x":w_x, "w_h":w_h, "bias":bias
                }
            )
            # index_dict = {
            #     "w_ix":w_ix_idx, "w_fx":w_fx_idx, "w_cx":w_cx_idx, "w_ox":w_ox_idx,
            #     "w_ih":w_ih_idx, "w_fh":w_fh_idx, "w_ch":w_ch_idx, "w_oh":w_oh_idx,
            #     "b_i":b_i_idx, "b_f":b_f_idx, "b_c":b_c_idx, "b_o":b_o_idx,
            # }
                
            def export_sacle_zeropoint(index_list, name):
                scaler_list = []
                zeropoint_list = []
                for idx in index_list:
                    if tensors[idx]["quantization"]['scale'] is not None:
                        scaler_list.append(np.array(tensors[idx]['quantization' ]['scale']))
                        zeropoint_list.append(np.array(tensors[idx]['quantization' ]['zero_point']))
                        # l.update({f"{idx_name}_scale":np.array(tensors[idx]['quantization' ]['scale'])})
                        # l.update({f"{idx_name}_zeropoint":np.array(tensors[idx]['quantization' ]['zero_point'])})
                    else:
                        # l.update({f"{idx_name}_scale":1})
                        # l.update({f"{idx_name}_zeropoint":0})
                        scaler_list.append(1)
                        zeropoint_list.append(0)
                l.update({f"{name}_scale":np.array(scaler_list)})
                l.update({f"{name}_zeropoint":np.array(zeropoint_list)})

            export_sacle_zeropoint([w_ix_idx, w_fx_idx, w_cx_idx, w_ox_idx], "w_x")
            export_sacle_zeropoint([w_ih_idx, w_fh_idx, w_ch_idx, w_oh_idx], "w_h")
            export_sacle_zeropoint([b_i_idx, b_f_idx, b_c_idx, b_o_idx], "bias")

            print("[INFO]>>> UNIDIRECTIONAL_SEQUENCE_LSTM ok!!!!")
...
is_qua = True
params_path = "../include/lstm_qua_params.h"
layers = read_tflite("../models/lstm_qua.tflite")


with open(params_path, "w") as f:

    f.write(f"#ifndef {os.path.basename(params_path)[:-2].upper()} \n")
    f.write(f"#define {os.path.basename(params_path)[:-2].upper()} \n\n")
    ...
    for layer in layers:
        print(f"[INFO]>>> layer:{layer['name']}")
        if layer['name'] == "UNIDIRECTIONAL_SEQUENCE_LSTM":

            w_b_list = ["w_x", "w_h", "bias"]
            f.write(f"{'//'*5}UNIDIRECTIONAL_SEQUENCE_LSTM{'//'*5} \n")
            for w_b in w_b_list:
                print(f"\t {w_b}:{layer[w_b].shape}")
                d_type = "int8_t" if (is_qua and w_b != "bias") else "float_t"
                s = f"{d_type} lstm_{w_b}[] = " + f"{layer[w_b].flatten().tolist()};\n".replace('[', '{').replace(']', '}')
                f.write(s)

                scale_ = f"{w_b}_scale"
                s = f"float lstm_{w_b}_s[] = " + f"{layer[scale_].flatten().tolist()};\n".replace('[', '{').replace(']', '}')
                f.write(s)

                zeropoint_ = f"{w_b}_zeropoint"
                s = f"float lstm_{w_b}_z[] = " + f"{layer[zeropoint_].flatten().tolist()};\n\n\n".replace('[', '{').replace(']', '}')
                f.write(s)

        elif layer['name'] == "FULLY_CONNECTED":
            f.write(f"{'//'*5}FULLY_CONNECTED{'//'*5} \n")
            s = f"float fc_w[] = " + f"{layer['weight'].flatten().tolist()};\n".replace('[', '{').replace(']', '}')
            f.write(s)

            s = "float fc_w_s = " + f"{layer['w_scale']} ; \n".replace('[', '{').replace(']', '}')
            f.write(s)
            s = "float fc_w_z = " + f"{layer['w_zeropoint']} ; \n\n".replace('[', '{').replace(']', '}')
            f.write(s)

            s = "float fc_b[] = " + f"{layer['bias'].tolist()};\n".replace('[', '{').replace(']', '}')
            f.write(s)
            s = "float fc_b_s = " + f"{layer['b_scale']}; \n".replace('[', '{').replace(']', '}')
            f.write(s)
            s = "float fc_b_z = " + f"{layer['b_zeropoint']}; \n\n".replace('[', '{').replace(']', '}')
            f.write(s)
